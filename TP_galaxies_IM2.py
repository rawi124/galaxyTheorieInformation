...
def load_galaxies(csv_file):
    ...
    return galaxies, emplacements, densites, logmass, metallicitys, agee, morphologique
...
def prepare_galaxies():
    ...
    cube, emplacements, densites, logMass, metallicity, agee, morphologie = load_galaxies("cube.csv")
    ...
    return cube, coords, emplacements, densites, logMass, metallicity, agee, morphologie
...
def main():
    ...
    galaxies, coords, emplacements, densites, logMass, meta, age, morphologie = prepare_galaxies()
    ...
    colorr = np.array(morphologie)

    """affichage du redshift"""
    colors = np.array([g.point[2] for g in galaxies])
    colors = (colors - min(colors)) / (max(colors) - min(colors))
    colors = abs(1-colors)
    red = discretisation_amplitude(colors)
    print(calcule_information_interaction(color, colorr, red))
...
def discretisation_geometrique(liste_valeurs_propriete):
    pas = 2
    bins = []
    depart = min(liste_valeurs_propriete)
    tmp = 0.001
    while tmp < max(liste_valeurs_propriete):
        bins += [tmp]
        tmp = tmp * pas
...
def discretisation_amplitude(liste_valeurs_propriete):
    #amplitude egale
    """
...
Commentez les résultats. Quelles variables sont corrélées avec le ty    h_xy = []
    for x,y in zip(liste_tirages_varX, liste_tirages_varY):
        h_xy.append((x,y))
    h_xy = np.array(h_xy,dtype="f,f")
    h_XY = calcule_entropy(calcule_distribution_proba(h_xy))#entropie jointepe
...
def calcule_distribution_proba(liste_tirages):
    ...
    probas = unique[1]
    return probas/(len(liste_tirages))
